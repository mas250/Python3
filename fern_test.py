...
def srs_print(S,n,rules):
    """Re-write an axiom according to given rules where S is the axiom
     ...
     chareter(s) to be re-written to"""
    
    for i in range (n):
        newaxiom = ''
        for i in S:                
            newaxiom += rules.get(i,i)      #if the charater in S is in the dictionary
        if i not in rules.keys():           #re-write accorind to the dictionary value
            newaxiom += i                   #otherwise leave the charater as it is
        S = newaxiom
...
def follow(S, n, t, step, stack, angle):
    """Turn string into turtle movement where S is the string to be drawn,
     ...
     is a dictionary with the key as the inital character and the value is the
     chareter(s) to be re-written to, step is the distance the turtle with move
     in a striaght line, and angle is the amount of degrees the turtle will rotate"""

    t.penup()
    t.setposition(0,-250)
    t.pendown()
    t.left(90)                              #better positioing for the fern
    for letter in S:
        
        if letter == 'F':
            forward(t,step)                 #for each charater in the string S
